  ...
  for root, drs, files in os.walk(p):
    ...
    for name in files:
      if not name.startswith(".") or name.startswith("~"):
        ...
        url_list.append("/".join([prefix] + os.path.split(root) + [quote(name)]))
...
if __name__=="__main__":
  ...
  with zipfile.ZipFile(os.path.join(dataset, "{0}_downloader.zip".format(dataset)), "w") as dszip:
    ...
    urls, rawsize = scan_files(DURL.format(dataset, filedir), path_to_files)
    ...
    dszip.writestr("download.sh", bat_file.encode("utf-8"))
    dszip.writestr("filelist.txt", bat_file.encode("utf-8"))
